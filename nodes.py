...
MODEL_NOT_LOADED_MSG = "Model is None. Please load the model first."
...
def _check_required_files(model_path: str) -> List[str]:
    """Helper to check required configuration files."""
    missing_required = []
...
def _check_weight_files(model_path: str) -> Tuple[bool, List[str]]:
    """Helper to check weight files."""
    missing_weights = []
    weight_found = False

    index_file_path = os.path.join(model_path, INDEX_FILE_NAME)
    if not os.path.exists(index_file_path):
        missing_weights.append(INDEX_FILE_NAME)
    else:
    ...
    return weight_found, missing_weights
...
def check_model_files_safe(model_path: str) -> Tuple[bool, List[str], List[str]]:
    ...
    if not os.path.exists(model_path):
        return False, REQUIRED_FILES, [INDEX_FILE_NAME]

    missing_required = _check_required_files(model_path)
    weight_found, missing_weights = _check_weight_files(model_path)

    success = not missing_required and weight_found
    return success, missing_required, missing_weights


def download_model(repo_name: str, local_path: str, token: str = None) -> bool:
...
def _verify_and_cleanup_download(local_path: str, repo_name: str) -> str:
    """Verify download and cleanup if failed."""
...
def ensure_model_available(repo_name: str) -> str:
    ...
    try:
        ...
        return _verify_and_cleanup_download(local_path, repo_name)
        ...
        elif "interpreter shutdown" in err_str:
...
def get_ovis_tokenizers(model):
    ...
    if text_tokenizer is None or visual_tokenizer is None:
        raise RuntimeError("Model tokenizers are None. Model may not be properly loaded.")
    ...
    def load_model(self, model_repo_id: str, device: str, dtype: str, trust_remote_code: bool):
        ...
        try:
            ...
            if local_model_path is None:
                raise RuntimeError(f"Model not available: {model_repo_id}")
            if device == "auto":
    ...
    def edit_image(self, model, image, prompt, steps, txt_cfg, img_cfg, seed):
        ...
        try:
            ...
            with torch.inference_mode():
                ...
                if len(images) == 0:
                    raise RuntimeError("No images found in generation output")
                gen_image = images[0]
